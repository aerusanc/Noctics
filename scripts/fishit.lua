...
style(topLine,{Size=UDim2.new(1,0,0,1),BackgroundColor3=C.line})
...
style(title, {BackgroundTransparency=1, Size=UDim2.new(1,-84,1,0), Position=UDim2.new(0,10,0,0), Text=" Noctics Hub", TextSize=15, Font=Enum.Font.GothamSemibold, TextColor3=C.text, TextXAlignment=Enum.TextXAlignment.Left})
...
local sbList = Instance.new("UIListLayout", sideWrap)
sbList.Padding = UDim.new(0,6)

local function makeSideButton(text)
    local b = Instance.new("TextButton", sideWrap)
    style(b, {Size=UDim2.new(1,0,0,28), Text=" "..text, TextXAlignment=Enum.TextXAlignment.Left, BackgroundColor3=C.pane2, TextColor3=C.text, TextSize=14, Font=Enum.Font.GothamSemibold})
    Instance.new("UICorner", b).CornerRadius = UDim.new(0,6)
    b.AutoButtonColor = false
    b.MouseEnter:Connect(function() if b.BackgroundColor3 ~= C.pane then tween(b,.08,{BackgroundColor3=C.pane}) end end)
    b.MouseLeave:Connect(function() if b ~= _G.__activeBtn then tween(b,.10,{BackgroundColor3=C.pane2}) end end)
    return b
end
...
cPad.PaddingRight=UDim.new(0,10); cPad.PaddingBottom=UDim.new(0,8)

local cTitle = Instance.new("TextLabel", content)
style(cTitle, {Size=UDim2.new(1,0,0,20), BackgroundTransparency=1, Text="Welcome", TextSize=15, Font=Enum.Font.GothamSemibold, TextColor3=C.text, TextXAlignment=Enum.TextXAlignment.Left})

local divider = Instance.new("Frame", content)
style(divider,{Size=UDim2.new(1,0,0,1), Position=UDim2.new(0,0,0,24), BackgroundColor3=C.line})
...
local FishIt = {} do
    ...
    function FishIt.Stop()
        flags.autoFishing=false; flags.cancelFishing=false; unequipAll()
    end

    function FishIt.CancelNow()
        flags.cancelFishing = true
    end

    function FishIt.SetAutoTeleport(on, name)
        flags.autoTeleport=on; flags.selectedEvent=name;
        if on and name then teleportToEvent(name) end
    end

    function FishIt.SetSelectedEvent(name)
        flags.selectedEvent = name
    end
...
local function makeToggle(parent, label, default, onChange)
    ...
    style(txt,{Size=UDim2.new(1,-64,1,0), BackgroundTransparency=1, Text=label, TextSize=14, Font=Enum.Font.Gotham, TextColor3=C.text, TextXAlignment=Enum.TextXAlignment.Left})
    ...
    local function render()
        ...
        tween(knob,.1,{Position = state and UDim2.new(1,-20,0.5,-9) or UDim2.new(0,2,0.5,-9), BackgroundColor3 = state and C.text or C.dim})
    end
    render()
...
local function makeContentDropdown(titleText)
    ...
    style(head, {Size=UDim2.new(1,0,0,28), Text=" "..titleText.." ⌄", TextXAlignment=Enum.TextXAlignment.Left, TextColor3=C.text, TextSize=14, BackgroundTransparency=1, Font=Enum.Font.GothamSemibold})
    ...
    local li = Instance.new("UIListLayout", container)
    li.Padding = UDim.new(0,6)
    ...
    local function addToggle(label, default, onChange) return makeToggle(container, label, default, onChange) end
    ...
    local function addSelector(label, listItems, defaultText, onPick)
        ...
        style(lbl, {Size=UDim2.new(.4,0,1,0), BackgroundTransparency=1, Text=label, TextSize=14, TextXAlignment=Enum.TextXAlignment.Left, TextColor3=C.text, Font=Enum.Font.Gotham})

        local btn = Instance.new("TextButton", row)
        style(btn, {Size=UDim2.new(.6,0,1,0), Position=UDim2.new(.4,0,0,0), Text=" "..(defaultText or "Choose…"), TextXAlignment=Enum.TextXAlignment.Left, BackgroundColor3=C.pane2, TextColor3=C.text, TextSize=14, Font=Enum.Font.Gotham})
        ...
        local pLi = Instance.new("UIListLayout", popup)
        pLi.Padding = UDim.new(0,2)
        ...
        for _,name in ipairs(listItems) do
            ...
            style(b, {Size=UDim2.new(1,0,0,22), Text=" "..name, TextXAlignment=Enum.TextXAlignment.Left, BackgroundColor3=C.pane2, TextColor3=C.text, TextSize=14, Font=Enum.Font.Gotham})
        ...
        return {
            get=function() return btn.Text:sub(2) end,
    ...
    local function addButtonRow(buttons) -- { {text=..., callback=function() end}, ...
        ...
        for _,b in ipairs(buttons) do
            local btn = Instance.new("TextButton", row)
            style(btn, {Size=UDim2.new(0, math.floor((content.AbsoluteSize.X-60)/#buttons), 1, 0), Text=b.text, BackgroundColor3=C.pane2, TextColor3=C.text, TextSize=14, Font=Enum.Font.Gotham})
            Instance.new("UICorner", btn).CornerRadius = UDim.new(0,6)
...
local function pageMain()
    ...
    -- Spacer
    local sp = Instance.new("Frame", cArea)
    style(sp,{Size=UDim2.new(1,0,0,8), BackgroundTransparency=1})
...
local function setOpen(v)
    ...
    if isOpen then
        ...
        task.delay(.16, function()
            if not isOpen then main.Visible = false; dock.Visible = true end
        end)
    end
end
